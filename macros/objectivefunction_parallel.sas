%macro objectivefunction_parallel(clusdset,flowdset,resdata,uratedat,earndat,inlib=OUTPUTS,outlib=OUTPUTS,otherlib=OUTPUTS,noprint=NO) ;
...
*******************************/
/*
proc contents data=&inlib..&flowdset.;
run;
proc contents data=&inlib..&resdata.;
run;
*/
proc sort data= &inlib..&flowdset. out= flows (rename=(home_cty =cty)); 
	by home_cty ; 
run; 

proc sort data = &inlib..&resdata. out=reslf (rename=(home_cty=cty));
	by home_cty ; 
run ;
...
%cluster_correlation(&inlib..&clusdset.,&inlib..&uratedat.,cluster,corr) ;
...
data urate;
    set urate; 
    meancorr_urate=meancorr_urate/&sdurate. ;
run;
/*
proc print data=urate;
    title 'urate';
run;
*/
/***********************
...
%cluster_correlation(&inlib..&clusdset.,&inlib..&earndat,cluster,corr) ;
...
data earnings;
    set earnings; 
    meancorr_earnings=meancorr_earnings/&sdearn. ;
run;    
    /*
...
proc print data=&outlib..objectivefn ;
    title 'objective function for &clusdset.' ;
run;
*/
/**********************
TURNING LOG BACK ON
...
%mend objectivefunction_parallel ;
